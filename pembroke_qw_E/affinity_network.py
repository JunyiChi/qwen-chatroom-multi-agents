...
# Build affinity matrix based on interaction frequency
def build_affinity_matrix(chat_log):
    ...
    for s1 in speakers:
        for s2 in speakers:
            all_scores.append(affinity[s1][s2])  # This will collect many zeros
    max_score = max(all_scores) if all_scores else 1

    matrix = np.zeros((len(speakers), len(speakers)))
    for i, s1 in enumerate(speakers):
        for j, s2 in enumerate(speakers):
            score = affinity[s1][s2] / max_score  # If max_score is 0, this will cause an error
            matrix[i][j] = score

    return speakers, matrix

# Plot social affinity network
...
if __name__ == "__main__":
    ...
    # Lower the threshold to show more connections
